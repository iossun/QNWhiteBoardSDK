 ...
 * 进入白板房间
 *
 * 执行此方法可以加入白板房间，加入成功后会收到{@link QNWhiteboardDelegate#onJoinSuccess(Room, RoomMember)}回调，
 * 加入失败则会收到{@link WhiteBoardListener#onJoinFailed(int)}回调。
 *
 * 如果已经加入成功但是发生了掉线，则SDK会尝试自动重连，并收到{@link QNWhiteboardDelegate#onReconnecting(int)}回调。
 * 重连次数可以通过{@link #setRetry(int)}修改，重连成功后会收到{@link QNWhiteboardDelegate#onReconnected()}，重连失败会收到
 * {@link QNWhiteboardDelegate#onDisconnected()}，此时必须用户手动调用此方法重新加入房间。
 *
...
-(void)screenShots:(id<QNWBScreenShotsDelegate>)delegate_ image:(UIImage *)thumbnail;
/**
 ...
 * @see QNWhiteboardDelegate#onCurrentBoardPageChanged(WhiteBoardPage)
 * @see QNWhiteboardDelegate#onBoardPageList(List)
 * @see QNWhiteboardDelegate#onBoardPageInfoChanged(WhiteBoardPage)
 */
-(void)newBoardPage;
